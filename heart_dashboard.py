...
<style>
    ...
    /* ── Result boxes ── */
    .result-yes {
        background: #2D1515; border: 2px solid #E53935;
        border-radius: 12px; padding: 20px; text-align: center;
        font-size: 1.3rem; font-weight: 700; color: #FF6B6B !important;
    }
    .result-no {
        background: #0D2016; border: 2px solid #2E7D32;
        border-radius: 12px; padding: 20px; text-align: center;
        font-size: 1.3rem; font-weight: 700; color: #56D364 !important;
    }

    /* ── Predict / Submit button — bright red, always visible ── */
    .stButton > button,
    ...
    /* ── Number input: +/- step buttons visible ── */
...
if page == "🏠  Overview":
    ...
    with c3:
        st.markdown(f"""<div class="metric-card">
            <div class="metric-value">{df['target'].sum()}</div>
            <div class="metric-label">With Disease</div></div>""", unsafe_allow_html=True)
    with c4:
        st.markdown(f"""<div class="metric-card">
            <div class="metric-value">{(df['target']==0).sum()}</div>
            <div class="metric-label">No Disease</div></div>""", unsafe_allow_html=True)
    with c5:
        st.markdown(f"""<div class="metric-card">
            <div class="metric-value">{acc_knn}%</div>
            <div class="metric-label">Best Accuracy</div></div>""", unsafe_allow_html=True)
    ...
    for col, name, acc, color, emoji in zip(
        ...
        with col:
            ...
            <div style="background:#161B22;border-radius:14px;padding:22px;
                ...
                <div style="font-size:1rem;font-weight:700;color:#8B949E">{emoji} {name}</div>
                <div style="font-size:2.5rem;font-weight:800;color:{color};margin:8px 0">{acc}%</div>
...
# ════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        with c1:
            ...
            ax.bar(['No Disease (0)', 'Has Disease (1)'],
                   [counts[0], counts[1]],
        ...
        # ── NEW: Sex vs Disease & Chest Pain vs Disease (from report Figs 3 & 4) ──
        st.markdown("---")
        c5, c6 = st.columns(2)
        with c5:
            fig, ax = plt.subplots(figsize=(6, 4))
            ...
            x = np.arange(2)
            w = 0.35
            bars1 = ax.bar(x - w/2, sex_disease[1], w, color='#F44336', label='Heart Disease', edgecolor='white')
            bars2 = ax.bar(x + w/2, sex_disease[0], w, color='#37474F', label='No Disease', edgecolor='white')
            for b in list(bars1) + list(bars2):
                ax.annotate(str(int(b.get_height())),
                            (b.get_x() + b.get_width()/2, b.get_height() + 3), ha='center', fontsize=9, fontweight='bold')
        ...
        with c6:
            ...
            x = np.arange(len(cp_disease))
            w = 0.35
            bars1 = ax.bar(x - w/2, cp_disease[1], w, color='#F44336', label='Heart Disease', edgecolor='white')
            bars2 = ax.bar(x + w/2, cp_disease[0], w, color='#37474F', label='No Disease', edgecolor='white')
            for b in list(bars1) + list(bars2):
                ax.annotate(str(int(b.get_height())),
                            (b.get_x() + b.get_width()/2, b.get_height() + 2), ha='center', fontsize=9, fontweight='bold')
            ax.set_xticks(x); ax.set_xticklabels(['Typical\n(0)', 'Atypical\n(1)', 'Non-ang\n(2)', 'Asymp\n(3)'])
            ax.set_title('5.3 Chest Pain Type vs Disease', fontweight='bold', fontsize=12)
            ax.set_ylabel('Count'); ax.legend()
            fig.tight_layout(); st.pyplot(fig); plt.close()

        # ── NEW: ST Slope & Major Vessels vs Disease (from report Figs 8 & 9) ──
        c7, c8 = st.columns(2)
        with c7:
            fig, ax = plt.subplots(figsize=(6, 4))
            slope_disease = df.groupby(['slope', 'target']).size().unstack(fill_value=0)
            x = np.arange(len(slope_disease))
            bars1 = ax.bar(x - w/2, slope_disease[1], w, color='#F44336', label='Heart Disease', edgecolor='white')
            bars2 = ax.bar(x + w/2, slope_disease[0], w, color='#37474F', label='No Disease', edgecolor='white')
            for b in list(bars1) + list(bars2):
                ax.annotate(str(int(b.get_height())),
                            (b.get_x() + b.get_width()/2, b.get_height() + 2), ha='center', fontsize=9, fontweight='bold')
        ...
        with c8:
            fig, ax = plt.subplots(figsize=(6, 4))
            ca_disease = df.groupby(['ca', 'target']).size().unstack(fill_value=0)
            x = np.arange(len(ca_disease))
            bars1 = ax.bar(x - w/2, ca_disease.get(1, pd.Series(0, index=ca_disease.index)), w,
                           color='#F44336', label='Heart Disease', edgecolor='white')
            bars2 = ax.bar(x + w/2, ca_disease.get(0, pd.Series(0, index=ca_disease.index)), w,
            ...
            ax.set_xticks(x); ax.set_xticklabels([str(i) for i in ca_disease.index])
    ...
    with tab2:
        ...
        colors = ['#E53935' if v > 0 else '#1565C0' for v in corr_target.values]
        ...
        # ── NEW: Feature–Target Direction Summary Table (Section 6.1 of report) ──
        st.markdown("---")
        st.markdown('<div class="section-title">6.1 Feature–Target Correlation Direction Summary</div>', unsafe_allow_html=True)
        corr_summary = pd.DataFrame({
            "Feature":   ["cp","thalach","thal","ca","oldpeak","exang","age","sex","trestbps","chol","fbs","restecg","slope"],
            ...
            "Key Insight": [
                ...
                "Disease patients avg 139 bpm vs 158 bpm healthy — impaired cardiac reserve",
        ...
        # ── NEW: ST Depression Clinical Thresholds (Section 3.2 of report) ──
        st.markdown("---")
        st.markdown('<div class="section-title">3.2 ST Depression (oldpeak) — Clinical Thresholds</div>', unsafe_allow_html=True)
        ...
        with m2:
            st.markdown(f"""<div class="metric-card" style="border-left-color:#E53935">
                <div class="metric-value" style="color:#FF6B6B">0.57 mm</div>
        ...
        with m3:
            st.markdown(f"""<div class="metric-card" style="border-left-color:#2E7D32">
                <div class="metric-value" style="color:#56D364">1.60 mm</div>
...
# ════════════════════════════════════════════════════════════
    ...
    ax.set_ylabel('Accuracy (%)'); ax.legend(); ax.set_title('Model Accuracy', fontweight='bold')
    fig.tight_layout(); st.pyplot(fig); plt.close()
...
# ════════════════════════════════════════════════════════════
    ...
    if submitted:
        ...
        for col, name, pred, prob, color, emoji in zip(
            [r1, r2, r3],
            ["KNN", "Logistic Regression", "Naive Bayes"],
            ...
            with col:
                verdict = "❤️ Has Disease" if pred == 1 else "💚 No Disease"
                bg = "#FFEBEE" if pred == 1 else "#E8F5E9"
                fc = "#C62828" if pred == 1 else "#1B5E20"
                st.markdown(f"""
                <div style="background:{bg};border:2px solid {color};border-radius:14px;
                            padding:20px;text-align:center">
                    <div style="font-size:1rem;font-weight:700;color:{color}">{emoji} {name}</div>
                    <div style="font-size:1.6rem;font-weight:800;color:{fc};margin:10px 0">{verdict}</div>
                    <div style="background:#ddd;border-radius:6px;height:8px;overflow:hidden;margin:8px 0">
                        <div style="background:{color};width:{prob:.0f}%;height:100%;border-radius:6px"></div>
                    </div>
                    <div style="font-size:0.85rem;color:{color};font-weight:600">Disease probability: {prob:.1f}%</div>
                </div>""", unsafe_allow_html=True)
        ...
        if votes >= 2:
            ...
            <div class="result-yes">
                ❤️ FINAL VERDICT: LIKELY HAS HEART DISEASE<br>
                <span style="font-size:0.9rem;font-weight:400">
                {votes}/3 models agree · Average disease probability: {avg_prob:.1f}%
                </span>
            </div>""", unsafe_allow_html=True)
        else:
            st.markdown(f"""
            <div class="result-no">
                💚 FINAL VERDICT: LIKELY NO HEART DISEASE<br>
                <span style="font-size:0.9rem;font-weight:400">
                {3-votes}/3 models agree · Average disease probability: {avg_prob:.1f}%
                </span>
            </div>""", unsafe_allow_html=True)
...
# ════════════════════════════════════════════════════════════
# PAGE 5 — CLINICAL SUMMARY  (Section 7 of report)
# ════════════════════════════════════════════════════════════
    ...
    observations = [
        ("⚖️ Balanced Dataset",
         f"51.3% disease (n=526) vs 48.7% no disease (n=499) — suitable for ML without aggressive resampling."),
        ("👥 Sex Distribution",
         f"Males = 69.6% of dataset (n=713), yet females show proportionally higher disease rate in this sample."),
        ("💤 Silent CAD Paradox",
         f"Asymptomatic chest pain (cp=0) is the most common type at ~48.5% — highlighting the 'silent' nature of CAD that standard screening misses."),
        ("🩸 Diabetes Prevalence",
         f"Only 14.9% of patients have elevated fasting blood sugar (>120 mg/dL) — predominantly non-diabetic sample."),
        ("📉 ST Depression (oldpeak)",
         f"Mean oldpeak: 1.07 mm overall — 0.57 mm in disease group vs 1.60 mm in no-disease group (difference of 1.03 mm). Note inverse coding in this dataset."),
        ("💓 Max Heart Rate",
         f"Disease patients average 139 bpm vs 158 bpm in healthy patients — a 19 bpm gap reflecting impaired cardiac reserve under stress."),
        ("🔬 Thalassemia Findings",
         f"Reversible defect (thal=3) appears in 40.0% of all patients and is strongly linked to a positive disease diagnosis."),
        ("🩺 Vessel Blockage",
         f"56.4% of patients have zero major vessels coloured by fluoroscopy, reflecting early-stage or single-event disease cases."),
    ]

    for title, text in observations:
        st.markdown(f"""
        <div style="background:#161B22;border-left:5px solid #E53935;
                    border-radius:10px;padding:16px 20px;margin-bottom:12px;
                    box-shadow:0 2px 8px rgba(0,0,0,0.3)">
            <div style="font-size:1rem;font-weight:700;color:#7EB3FF;margin-bottom:6px">{title}</div>
            <div style="font-size:0.92rem;color:#C9D1D9;line-height:1.5">{text}</div>
        </div>""", unsafe_allow_html=True)
    ...
    features = [
        ("cp — Chest Pain Type",       "#1565C0", "Strong categorical predictor. Typical angina (type 0) and asymptomatic (type 3) show distinct disease associations."),
        ("thalach — Max Heart Rate",    "#2E7D32", "Patients with disease average 139 bpm vs 158 bpm without — impaired stress response is a hallmark of CAD."),
        ("thal — Thalassemia",          "#E53935", "Reversible defect on nuclear scan is the single strongest non-invasive indicator of ischaemia."),
        ("ca — Fluoroscopy Vessels",    "#7B1FA2", "Number of visible blocked vessels directly reflects anatomical disease burden."),
        ("oldpeak — ST Depression",     "#E65100", "Magnitude of exercise-induced ST depression is a critical continuous marker of myocardial ischaemia."),
        ("exang — Exercise Angina",     "#00695C", "Presence of exercise-induced chest pain is a direct symptom of ischaemia under physical stress."),
    ]
    ...
    for i, (fname, color, desc) in enumerate(features):
        ...
        with col:
            st.markdown(f"""
            <div style="background:#161B22;border-top:4px solid {color};
                        border-radius:10px;padding:14px 18px;margin-bottom:12px;
                        box-shadow:0 2px 8px rgba(0,0,0,0.3)">
                <div style="font-size:0.95rem;font-weight:700;color:{color};margin-bottom:6px">{fname}</div>
                <div style="font-size:0.88rem;color:#C9D1D9;line-height:1.5">{desc}</div>
            </div>""", unsafe_allow_html=True)

    st.markdown("---")

    # ── 7.2 ST Depression Final Summary ──
    st.markdown('<div class="section-title">7.2 ST Depression (oldpeak) — Final Summary</div>', unsafe_allow_html=True)
    st.markdown("""
    <div style="background:#1C2128;border:1px solid #444C56;border-radius:12px;padding:20px 24px;line-height:1.7;color:#C9D1D9">
        <b style="color:#7EB3FF">oldpeak</b> measures exercise-induced ST segment depression in millimetres.
        A higher value traditionally indicates greater myocardial ischaemia. In this dataset's coding, however,
        it is <b style="color:#FF6B6B">inversely correlated</b> with the disease label — suggesting disease patients
        had less severe depression at testing (possibly earlier-stage disease or different testing protocols).
        Combined with <b style="color:#7EB3FF">slope</b>, it remains one of the most clinically interpretable
        features in any CAD risk model.
    </div>""", unsafe_allow_html=True)
    ...
    limitations = [
        ("🔀 Multi-source Merging",    "Records merged from Cleveland, Hungary, Switzerland, and Long Beach VA — potential batch effects between institutions."),
        ("❓ Thal Coding Anomaly",     "thal=0 appears for 7 patients, possibly representing missing or erroneous data not present in the original Cleveland set."),
        ("⚠️ CA Value Out of Range",   "CA values of 4 appear in 18 records; the original dataset caps at 3, suggesting data-entry issues or extended coding."),
        ("📊 Binary Target Simplification", "Binary target (0/1) simplifies a spectrum of disease severity into a single label — nuance of mild vs severe CAD is lost."),
        ("📅 Single Visit Snapshot",   "All features are from a single clinical visit; longitudinal patient dynamics and disease progression are not captured."),
    ]

    for title, text in limitations:
        st.markdown(f"""
        <div style="background:#161B22;border-left:5px solid #444C56;
                    border-radius:10px;padding:14px 18px;margin-bottom:10px">
            <div style="font-size:0.95rem;font-weight:700;color:#8B949E;margin-bottom:4px">{title}</div>
            <div style="font-size:0.88rem;color:#C9D1D9;line-height:1.5">{text}</div>
        </div>""", unsafe_allow_html=True)

    st.markdown("---")
    st.info("⚠️ This report is for educational and academic purposes only. Not intended as clinical medical advice. Always consult a qualified cardiologist.")
